 ...
 * @ Description: Assignment 05 - COMP 2511
 */
...
int main(void)
{
    ...
    // After this, we shuffle deck every time we do not have enough undealt cards
...
{
    // Student implements this function
    for (size_t i = FALSE; i < CARDS; ++i)
    {
        size_t j = rand() % CARDS;
        swap(&wDeck[i], &wDeck[j]);
    }
}
...
int isPair(const unsigned int hand[])
{
    ...
    // Iterate through all of the frequency counts to find out if
    //     any face card occurred 2 times in the hand
    for (size_t face = 0; face < FACES; ++face)
    {
        if (faceCnt[face] == 3)
        {
        ...
        else if (faceCnt[face] == 2)
        {
            pairCounter++;
        }
    }

    // Must have exactly **one** pair and no three-of-a-kind
    return (pairCounter == 1 && trioCounter == 0) ? TRUE : FALSE;
}
...
int isTwoPair(const unsigned int hand[])
{
    ...
    // We must have exactly 2 pairs to return TRUE.
...
int isThreeOfAKind(const unsigned int hand[])
{
    ...
    // We must find exactly **one** trio to have three-of-a-kind
    return (trioCounter == 1 && pairCounter == 0) ? TRUE : FALSE;
}
...
int isStraight(const unsigned int hand[])
{
    ...
    // Iterate through all of the frequency counts to find out if
    //     any face card occurred only 1 time in the hand and add it to the counter.
    for (size_t face = 0; face < FACES; ++face)
    {
        if (faceCnt[face] == 1)
        {
            seqCounter++;
            // if we identify a regular sequence of 5 cards in a row at any point, we exit the function and return true
            if (seqCounter == 5)
            {
                return TRUE;
            }
    ...
    // The edge case that has an issue needs to be hard coded, and will check a sequence from 10 through Ace
...
int isFlush(const unsigned int hand[])
{
    ...
    // In other words, it calculates how many hearts, spades, clubs and diamonds are in the same hand.
    for (size_t card = 0; card < HAND_SIZE; ++card)
    {
    ...
    // Iterate through all of the frequency counts to find out if
    //     any suit card occurred 5 times in the hand
...
int isFullHouse(const unsigned int hand[])
{
    ...
    // Iterate through all of the frequency counts to find out if
    //     any face card occurred 2 times in the hand and another has ocurred 3 times in the hand
    for (size_t face = 0; face < FACES; ++face)
    {
        if (faceCnt[face] == 3)
        {
    ...
    // For a full house, we must have exactly **one** pair and **one** trio
    return (trioCounter == 1 && pairCounter == 1) ? TRUE : FALSE;
}


//OPTIONAL ROYAL FLUSH COUNT
//Return TRUE if the poker hand is a royal flush
int isRoyalFlush(const unsigned int hand[])
{
    ...
    for (size_t suit = 0; suit < SUITS; ++suit)
    {
        if (suitCnt[suit] == 5 && ((faceCnt[0] && faceCnt[9] && faceCnt[10] && faceCnt[11] && faceCnt[12])))
        {
            return TRUE;
        }
    }
